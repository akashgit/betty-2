...
class UserModelDB:
    ...
    async def set_preference(
        ...
        await self.db.execute(
            ...
            """,
            (category, key, value, confidence, now, project_scope),
        )
        await self.db.commit()

    async def get_preference(
        self, category: str, key: str, project_scope: str | None = None
    ) -> dict[str, Any] | None:
        """Get a single preference."""
        async with self.db.execute(
            """
            SELECT value, confidence, evidence_count, last_seen
            FROM user_preferences
            WHERE category = ? AND key = ? AND project_scope IS ?
            """,
            (category, key, project_scope),
        ) as cursor:
            row = await cursor.fetchone()
            if not row:
                return None
    ...
    async def get_preferences_by_category(
        self, category: str, project_scope: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        if project_scope is not None:
            ...
            query += " AND project_scope IS NULL"
    ...
    async def save_session(
        ...
        await self.db.execute(
            ...
            INSERT INTO session_summaries
                (session_id, project_dir, started_at, ended_at, goal, outcome,
                 tools_used, files_touched, decisions_made, patterns_observed)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            (
                ...
                json.dumps(patterns_observed or []),
            ),
        )
        await self.db.commit()
    ...
    async def get_sessions_for_project(
        self, project_dir: str, limit: int = 50
    ) -> list[dict[str, Any]]:
    ...
    async def record_approval(
        ...
        await self.db.execute(
            ...
            ON CONFLICT(tool_name, action_pattern, project_scope) DO UPDATE SET
                ...
                last_seen = excluded.last_seen
            """,
            (tool_name, action_pattern, decision, now, project_scope),
        )
        await self.db.commit()

    async def get_approval_pattern(
        self, tool_name: str, action_pattern: str, project_scope: str | None = None
    ) -> dict[str, Any] | None:
        """Look up a previous approval decision."""
        async with self.db.execute(
            """
            SELECT decision, count, last_seen
            FROM approval_patterns
            WHERE tool_name = ? AND action_pattern = ? AND project_scope IS ?
            """,
            (tool_name, action_pattern, project_scope),
        ) as cursor:
            row = await cursor.fetchone()
            if not row:
                return None
    ...
    async def add_policy(
        ...
        """Add an organizational policy. Returns the policy ID."""
        cursor = await self.db.execute(
            """
            INSERT INTO org_policies (policy_type, rule, description, source, project_scope)
            VALUES (?, ?, ?, ?, ?)
            """,
    ...
    async def get_policies(
        self, policy_type: str | None = None, project_scope: str | None = None
    ) -> list[dict[str, Any]]:
        """Get policies, optionally filtered by type and scope."""
    ...
    async def log_escalation(
        ...
        await self.db.execute(
            """
            INSERT INTO escalation_log (session_id, question, response, channel, response_time_seconds, timestamp)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
